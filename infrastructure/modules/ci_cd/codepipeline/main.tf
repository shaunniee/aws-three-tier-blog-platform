...
resource "aws_iam_role_policy" "codepipeline_policy" {
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      { Effect = "Allow", Action = ["codebuild:BatchGetBuilds", "codebuild:StartBuild"], Resource = var.codebuild_frontend_proj_name },
      { Effect = "Allow", Action = ["s3:GetObject", "s3:GetObjectVersion", "s3:PutObject"], Resource = ["${var.s3_artifact_arn}/*"] }
    ]
  })
}
